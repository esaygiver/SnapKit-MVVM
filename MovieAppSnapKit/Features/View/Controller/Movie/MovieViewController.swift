...
//    private let mainView = MovieView()
    private lazy var searchBar: UISearchBar = {
        ...
        searchBar.placeholder = K.searchMovies.localized()
        searchBar.configure()
        return searchBar
    }()
    ...
    private let emptyMovieTitleLabel: UILabel = {
        ...
        label.backgroundColor = .clear
        label.textAlignment = .center
        label.text = K.noMovieResults.localized()
    ...
    private let emptyPersonTitleLabel: UILabel = {
        ...
        label.backgroundColor = .clear
        label.textAlignment = .center
        label.text = K.noCastResults.localized()
        label.numberOfLines = 0
        return label
    }()
    ...
    private var screenState: ScreenState? {
        didSet {
            ...
            case .loaded:
                ...
                categoryTitleLabel.text = K.popularMovies.localized()
...
extension MovieViewController: MovieViewControllerOutput {
    ...
    func getSearchedMovies(results: MovieModel) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            self.categoryTitleLabel.text = "'\(self.searchedTitle)' \(K.movieResults.localized()) -> \(results.totalNumber ?? 0)"
        }
    }
    ...
    func getSearchedPersons(results: PersonModel) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            self.castTitleLabel.text = "'\(self.searchedTitle)' \(K.castResults.localized()) -> \(results.totalNumber ?? 0)"
        }
    }
